...
class TestDevice(unittest.TestCase):
    ...
    def test_display_info(self):
        ...
        print("1)Testcase for test display_info ")
        print(self.device1.display_info())
        print("\n")
        print(self.device2.display_info())

    def test_apply_discount(self):
        self.device1.apply_discount(10)
        self.assertEqual(self.device1.price, 899)
        print("\n2)Testcase for test apply_discount ")
        print(f"With discounted Price: {self.device1.price}\n")
    ...
    def test_is_available(self):
        ...
        print(f"{self.device1.name} is available: {self.device1.is_available(5)}")
        print(f"{self.device2.name} is available: {self.device2.is_available(10)}\n")

    def test_reduce_stock(self):
        print(f"\n4)Initial stock for {self.device1.name}: {self.device1.stock}")
        print(f"  Initial stock for {self.device2.name}: {self.device2.stock}")
        self.assertTrue(self.device1.reduce_stock(5))
        self.assertFalse(self.device2.reduce_stock(10))
        print("\nTestcase for test reduce_stock ")
        print(f"Reduced stock for {self.device1.name}: {self.device1.stock}")
        print(f"Reduced stock for {self.device2.name}: {self.device2.stock}\n")

class TestSmartphone(unittest.TestCase):
    def setUp(self):
        self.smartphone = Smartphone("Iphone 13", 999, 10, 24, "6.1 inch", "19 hours")
        print(self.smartphone.display_info())

    def test_make_call(self):
        self.assertEqual(self.smartphone.make_call("996709785"), "Dialing ... 996709785 Call in progress...")
        print("\n5)Testcase for test make_call ")
        print(self.smartphone.make_call("996709785"))
    ...
    def test_instaling_app(self):
        ...
        print(self.smartphone.install_app("Instagram"))
...
class TestTablet(unittest.TestCase):
    ...
    def test_browse_internet(self):
        ...
        print(self.tablet.browse_internet("www.google.com"))
    ...
    def test_use_touchscreen(self):
        ...
        print(Tablet.use_touchscreen("interface"))
...
class TestCart(unittest.TestCase):
    ...
    def test_add_device(self):
        ...
        print(f"Added items: {len(self.cart.items)}")
    ...
    def test_remove_device(self):
        ...
        print(f"Remaining items: {self.cart.items}")
